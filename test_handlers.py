...
Also covers: empty-body guard never calls the LLM, HTML escaping of card fields,
and that send edits the preview card (the one carrying the Send button), marks it
sent, strips the keyboard, and is guarded against double-send.
"""
...
class FakeBot:
    ...
    async def edit_message_text(self, chat_id, message_id, text, parse_mode, reply_markup=None):
...
async def test_route_read_removes_unread_and_edits(tmp_path):
    ...
    assert bot.edits[0] == {"chat_id": 123456789, "message_id": 555,
                            "text": "✅ <b>Read</b>", "reply_markup": None}
    state.close()
...
async def test_route_cancel_edits_card_and_drops_draft(tmp_path):
    gmail = FakeGmail()
    h, bot, state = _handlers(tmp_path, gmail)
    state.save_draft("DKEY", Draft(text="x", thread_id="T", last_msg_id="LM",
                                   chat_id=123456789, tg_message_id=1, created_at=_now_ms()))
    await h.route(parse_callback("gmail:cancel:DKEY"), tg_message_id=601)
    assert gmail.removed == [] and gmail.trashed == []
    # Edits the card in place (not a new message) and strips the keyboard.
    assert bot.edits[0]["message_id"] == 601
    assert bot.edits[0]["text"] == "❌ <b>Cancelled</b>"
    assert bot.edits[0]["reply_markup"] is None
    # Draft dropped so a stale Send tap afterwards can't resend.
    assert state.get_draft("DKEY") is None
    state.close()
...
async def test_send_edits_preview_card_marks_sent_strips_keyboard(tmp_path):
    gmail = FakeGmail()
    h, bot, state = _handlers(tmp_path, gmail)
    # Draft stores the ORIGINAL new-email card id (1000); the preview card that
    # carries the Send button is a separate message (the callback's 9999).
    ...
    edit = bot.edits[0]
    # Edits the card that carried the Send button (9999), NOT the new-email card (1000).
    assert edit["message_id"] == 9999
    assert "📨 <b>Reply sent</b>" in edit["text"]
    assert "reply text" in edit["text"]     # sent text preserved for context
    assert edit["reply_markup"] is None      # inline keyboard removed
    assert state.get_draft("DKEY") is None   # draft consumed -> cannot re-send
    state.close()


async def test_send_long_reply_is_trimmed_in_confirmation(tmp_path):
    gmail = FakeGmail()
    h, bot, state = _handlers(tmp_path, gmail)
    long_text = "x" * 900
    state.save_draft("DKEY", Draft(text=long_text, thread_id="T7", last_msg_id="LM7",
                                   chat_id=123456789, tg_message_id=1000, created_at=_now_ms()))
    await h.route(parse_callback("gmail:send:T7:DKEY"), tg_message_id=9999)
    assert gmail.sent_replies == [("T7", "LM7", long_text)]  # full text still SENT
    assert "…" in bot.edits[0]["text"]  # confirmation card trims the display copy
    state.close()


async def test_send_double_tap_does_not_resend(tmp_path):
    gmail = FakeGmail()
    h, bot, state = _handlers(tmp_path, gmail)
    state.save_draft("DKEY", Draft(text="reply text", thread_id="T7", last_msg_id="LM7",
                                   chat_id=123456789, tg_message_id=1000, created_at=_now_ms()))
    await h.route(parse_callback("gmail:send:T7:DKEY"), tg_message_id=9999)
    await h.route(parse_callback("gmail:send:T7:DKEY"), tg_message_id=9999)  # stale re-tap
    assert gmail.sent_replies == [("T7", "LM7", "reply text")]  # sent exactly once
    assert bot.edits[1]["text"] == "📨 <b>Already sent</b>"
    assert bot.edits[1]["reply_markup"] is None
    state.close()


async def test_send_missing_draft_shows_already_sent(tmp_path):
    gmail = FakeGmail()
    h, bot, state = _handlers(tmp_path, gmail)
    await h.route(parse_callback("gmail:send:T:DKEY"), tg_message_id=1)
    assert gmail.sent_replies == []  # nothing sent
    assert bot.edits[0]["text"] == "📨 <b>Already sent</b>"
    state.close()


async def test_send_failure_restores_draft_for_retry(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        await h.route(parse_callback("gmail:send:T7:DKEY"), tg_message_id=9999)
    # Draft restored so the still-visible card can be retried.
    assert state.get_draft("DKEY") is not None
    state.close()
