...
class State:
    ...
    def pop_draft(self, msg_id: str) -> Draft | None:
        """Atomically fetch and delete a draft; return it, or None if absent.

        The read + delete run with no ``await`` between them, so on the single
        asyncio loop this is an atomic claim: two near-simultaneous Send taps
        cannot both obtain the same draft, which is the double-send guard.
        """
        row = self._conn.execute(
            "SELECT data FROM drafts WHERE msg_id = ?", (msg_id,)
        ).fetchone()
        if row is None:
            return None
        self._conn.execute("DELETE FROM drafts WHERE msg_id = ?", (msg_id,))
        self._conn.commit()
        return Draft(**json.loads(row["data"]))

    def delete_draft(self, msg_id: str) -> None:
        """Remove a draft if present (no-op when absent)."""
        self._conn.execute("DELETE FROM drafts WHERE msg_id = ?", (msg_id,))
        self._conn.commit()
