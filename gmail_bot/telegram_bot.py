...
class Handlers:
    ...
    async def _edit(self, message_id: int, text: str) -> None:
        await self._bot.edit_message_text(
            chat_id=self._chat_id, message_id=message_id, text=text, parse_mode="HTML"
        )
    ...
    async def handle_send(self, cb: Callback, tg_message_id: int) -> None:
        ...
        draft = self._state.get_draft(key)
        if draft is None:
            await self._edit(tg_message_id, error_card(f"Draft not found: {html_escape(key)}"))
            return
        await asyncio.to_thread(
            self._gmail.send_reply, draft.thread_id, draft.last_msg_id, draft.text
        )
        # Status edit targets the ORIGINAL card from the stored draft (spec 4.6).
        await self._edit(draft.tg_message_id, STATUS_REPLY_SENT)
